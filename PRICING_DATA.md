## Cursor

- Hobby - free
- Pro - $20/mo
- Pro+ - $60/mo
- ultra - $200/mo
- Teams - $40/user/mo
- Enterprise - Custom

Verified - 2026-05-07
Link - https://cursor.com/pricing

## Github copilot

- Free - $0 USD
- Pro - $10 USD per user/month
- Pro+ - $39 USD per user/month
- Business - $19 USD per user/month
- Enterprise - $39 USD per user/month

Verified - 2026-05-07
Link - https://github.com/features/copilot/plans

## Claude

- Free - $0
- Pro - $17
- Max - from $100
- Team standard seat - $20
- Team premium seat - $100
- Enterprise - $20/seat

Verified - 2026-05-07
Link - https://claude.com/pricing

## ChatGPT

- Free - ₹0/month
- Go - ₹399/month
- Plus - ₹1,999/month
- Pro - ₹10,699/month
- Business ChatGPT & Codex - usage pricing
- Business Codex - ₹1800/user/month
- Enterprise - custom pricing

Verified - 2026-05-07
Link - https://chatgpt.com/pricing/?utm_source=chatgpt.com

## Anthropic API

## Anthropic API

- API direct: Pay per token (no fixed plan)
  - Claude Opus 4: $5.00 input / $25.00 output per 1M tokens
  - Claude Sonnet 4.6: $3.00 input / $15.00 output per 1M tokens
  - Claude Haiku 4.5: $1.00 input / $5.00 output per 1M tokens

Verified - 2026-05-07
Link - https://platform.claude.com/docs/en/about-claude/pricing

## OpenAI API

- API direct: Pay per token (no fixed plan)
- GPT-5.5: $5.00 input / $30.00 output per 1M tokens
- GPT-5.4: $2.50 input / $15.00 output per 1M tokens
- GPT-5.4 mini: $0.75 input / $4.50 output per 1M tokens

Verified - 2026-05-07
Link - https://openai.com/api/pricing/

## Gemini

- plus: ₹399/mo
- Pro: ₹975/mo
- Ultra: ₹24,500/mo

Verified - 2026-05-07
Link - https://one.google.com/ai?utm_source=gemini&utm_medium=web&utm_campaign=gemini_ail_upsell_zero_state_banner&g1_landing_page=75

## Windsurf

- Free - $0
- Pro - $20
- Max - $200
- Team - $40 per user/month
- Enterprise - Custom pricing

Verified - 2026-05-07
Link - https://windsurf.com/pricing