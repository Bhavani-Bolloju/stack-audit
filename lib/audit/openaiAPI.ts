...
function auditOpenAIAPI(
  ...
  if (useCase !== "coding") {
    ...
    return {
      ...
      reason: `Openai API is built for coding — your use case (${useCase}) is better served by ${alternatives.join("/")}.`
    };
  }
  ...
  if (monthlySpend > CREDITS_THRESHOLD) {
    return {
      ...
      recommendedAction: "Get openai API through Credex discounted credits",
  ...
  return {
    ...
    estimatedMonthlySavings: 0,
    reason:
      "Your Openai API setup looks right-sized for your team and use case."
  };
}
