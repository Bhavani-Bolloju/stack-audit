...
const PRICES = TOOL_PRICES["chatgpt"];
...
function auditChatGPT(
  ...
  if (
    ...
    const lowerTier = getLowerTier("chatgpt", plan);
  ...
  if (useCase === "coding") {
    const alternatives = getAlternativeTools("chatgpt", useCase);
